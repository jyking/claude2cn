...
5. 术语表（与项目既有译文一致）：worktree→工作树、artifact→构件、plugin→插件、skill→技能、slash command→斜杠命令、seat→席位、subagent→子代理、sandbox→沙箱、usage→用量、pull request→拉取请求、commit→提交、model→模型、session→会话、marketplace→市场、agent→代理。
